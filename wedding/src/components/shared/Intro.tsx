  ...
  return (
    <Section className={cx('container')}>
      ...
      <div className={cx('wrap-location')}>
        {/* eeee는 요일을 가리킴 ex) Sunday 영어로 나옴 date-fns/locale 에서 ko를 불러오면 한글로 나옴 */}
        <span>
      ...
      {/* db.json 보면 &lt;, &gt; 와 같은 특수문자들이 사용되어 있는데 이럴때는 div의 dangerouslySetInnerHTML 속성을 사용해야 함 */}
      {/* dangerouslySetInnerHTML 속성은 string을 가지고 html 태그를 사용할 수 있게 해줌  */}
      {/* But 보안상의 이슈로 지금은 사용안함!!!!!!!!! */}
      {/* 지금은 그냥 db.json 에서 부등호(< , >)로 바꿔버림 */}
      {/* <div dangerouslySetInnerHTML={{ __html: message }} /> */}

      {/* /n 은 줄바꿈을 의미하는데 db.json에 있는 /n 줄바꿈을 지원할 수 있도록 Text 컴포넌트를 만들어 볼 거임 */}
      <Text>{message}</Text>
    </Section>
  )
}
